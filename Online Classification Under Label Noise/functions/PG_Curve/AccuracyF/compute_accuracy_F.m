function [confus,accuracy,precision,recall,F,A_minus,Gmeans,numcorrect] = compute_accuracy_F (actual,pred,classes)
...
for i=1:length(classes)
    ...
    if nargout>=6
            A_minus(i) = ...
                (length(actual) - sum(confus(i,:)) - sum(confus(:,i)) + confus(i,i))/(length(actual) - sum(confus(:,i)));
    end
    
    if nargout>=7
        Gmeans(i) = sqrt(recall(i)*A_minus(i));
    end
